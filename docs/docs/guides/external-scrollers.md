...
Use `VirtualizedContent` when another component creates and owns the native ScrollingFrame. This is useful for containers such as sheets that already provide scrolling behavior and expose a ref.

`VirtualizedContent` renders inside the existing scrolling canvas. It reuses the same virtualization controller, dimensions, keys, and renderers as `Components.ScrollingFrame`, but it never creates or configures another ScrollingFrame.
...
return React.createElement("ScrollingFrame", {
    ...
    List = React.createElement(UltimateList.Components.VirtualizedContent, {
...
- `VirtualizedContent` must be rendered inside the referenced ScrollingFrame's canvas. Do not place it beside the ScrollingFrame or inside another ScrollingFrame.
