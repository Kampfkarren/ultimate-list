...
**Props**
- `dataSource: DataSources.DataSource<T>`: The [data source](./core-concepts/data-sources) we are creating a list for.
- `dimensions: Dimensions.Dimensions<T>`: The [dimensions](./core-concepts/dimensions) that determine each item's size and absolute canvas position.
- `renderer: Renderers.Renderer<T>`: The [renderer](./core-concepts/renderers) that determines what an item looks like.
- `direction: "x" | "y"`: The dominant scrolling direction. This must agree with the external ScrollingFrame.
- `scrollingFrameRef: React.RefObject<ScrollingFrame?>`: A ref that points to the ancestor ScrollingFrame while this component is mounted.
