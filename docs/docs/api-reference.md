...
- `config.primaryType: string`: The dominant item type. Pre-allocated slots are created with this type for optimal initial rendering. Slots for other types are created on demand as those items enter the viewport.
